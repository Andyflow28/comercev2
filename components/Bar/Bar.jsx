...
const Bar = () => {
  const open =
    "bg-violet-900 flex items-center flex-col sm:flex-row sm:justify-around sm:border-b-4 border-violet-900";
  const close =
    "bg-violet-900 flex items-center flex-col sm:flex-row sm:justify-around sm:border-b-4 border-violet-900 hidden sm:flex";
  const [bar, setBar] = useState(close);

  return (
    <div className="sm:flex flex-col">
      <div
        className="mx-auto flex justify-center sm:hidden"
        onClick={() => {
          if (bar === close) {
            setBar(open);
          } else {
